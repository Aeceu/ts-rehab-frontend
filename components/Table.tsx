...
const TableTask: React.FC<TabelTaskProps> = ({ pending, completed, todos }) => {
  ...
  useEffect(() => {
    const getRandomTasks = () => {
      ...
      const dailyActivities = activitiesData.activities.find(
        (category: any) => category.category === "Mental Well-being"
      );
      const dailyTasks = dailyActivities?.activities.find(
      ...
      const weeklyActivities = activitiesData.activities.find(
        (category: any) => category.category === "Mental Well-being"
      );
      const weeklyTasks = weeklyActivities?.activities.find(
        (activity: any) => activity.frequency === "Weekly"
      )?.tasks;
      if (weeklyTasks) {
      ...
      const monthlyActivities = activitiesData.activities.find(
        (category: any) => category.category === "Mental Well-being"
      );
      const monthlyTasks = monthlyActivities?.activities.find(
        (activity: any) => activity.frequency === "Monthly"
      )?.tasks;
      if (monthlyTasks) {
  ...
  const handlePostTasks = async () => {
    try {
      ...
      const res = await axios.post(`${baseUrl}/uncomplete`, {
        id,
        randomTasks,
      });
    ...
    <main className="flex flex-col gap-4">
      ...
      <div className="flex gap-4 lg:flex-row flex-col-reverse">
        ...
        <div className="w-full flex-col xl:flex-row flex gap-4">
          <div
            className={`w-full xl:w-1/2 bg-inherit text-inherit   flex flex-col gap-4`}
          >
            <Tabs
              defaultValue="pending"
              className={`w-full bg-inherit text-inherit   flex flex-col gap-4`}
            >
