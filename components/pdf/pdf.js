      ...
      return `Username: ${user.email}\n\nTODOS:\n${user.Todos.map(
        (item) =>
          `- Title: ${item.title}, Description: ${
            item.description
          }, Time: ${createdDate(item.createdAt)}`
    ...
    <div>
      <div>
        <div className="w-full flex item-center justify-center p-2">
          ...
          >
            {loader ? <span>Downloading</span> : <span>Download Receipt</span>}
          </button>
        </div>
        <div className="w-full flex item-center justify-center p-2">
        ...
        {userData && (
          <div className="w-[450px] flex flex-col m-t-[20px] border-2 border-[#e4e4e4]">
            ...
            <main>
              ...
              <div className="flex flex-col">
                ...
                {userData.Todos?.map((item) => {
                  return (
                    <div
                      key={item._id}
                      className="flex border-b-2 px-2 text-[.8rem]"
                    >
                      <div className="flex flex-col items-center">
                        <h1 className="text-gray-500 h-[50px] w-[100px] border-r-2 flex items-center justify-center">
                          title
                        </h1>
                        <h1 className="text-gray-500 h-[50px] w-[100px] border-r-2 flex items-center justify-center">
                          description
                      ...
                      <div className="flex flex-col w-full h-full text-[.8rem]">
                        <p className="flex items-center px-2 h-[50px]">
                          {item.title}
                        </p>
                        <p className="flex items-center px-2 h-[50px]">
                          {item.description}
              ...
              <div className="flex flex-col">
                <h1 className="border-b-2 p-4 px-2 border-black">
                  PENDING TASKS
                </h1>
            ...
            <footer>
              <div className="flex  justify-center items-center text-[16px] gap-2 p-4 text-slate-600">
                @2023 <p className="text-[#25b953]">Rehabify.</p> All right
                reserved.
              </div>
            </footer>
          </div>
